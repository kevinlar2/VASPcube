...
#Zs = list(set(atom_types))
elements = [element_dict[Z] for Z in Zs]
num_atoms = [atom_count for Z, atom_count in atom_counts.items()]

line_index = 6 + total_atoms
for line in cube_data[line_index:]:
    density.extend(map(float, line.split()))

density = np.array(density[:nx * ny * nz])
...
with open(outputfilename, 'w') as f:
    ...
    f.write(f"{nx:>6} {ny:>6} {nz:>6}\n")
    i = 1
    for z in range(nz):
        for y in range(ny):
            for x in range(nx):
                itr = x * nz * ny + y * nz + z
                datum = vol * recip_e_units * density[itr]
                f.write(f" {datum:18.11E} ")
                if i % 5 == 0:
                    f.write("\n")
                i += 1

print("PARCHG file conversion complete.")
